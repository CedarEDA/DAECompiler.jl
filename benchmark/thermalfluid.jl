...
function (this::CircularWall{Ne, Nn})(inner_heatport, outer_heatport) where {Ne, Nn}
    ...
    foreach(T->initial!(T - this.T0), Tn)

    dn = (this.d, (this.d .+ 2.0 .* cumsum(this.t_layer))...)
    Cn = ((Cn_circular_wall_inner.(dn[1:Ne], dn[2:Nn], this.cp, this.ρ) .* this.dx)..., 0.)
    Cn = Cn .+ (0., (Cn_circular_wall_outer.(dn[1:Ne], dn[2:Nn], this.cp, this.ρ) .* this.dx)...)
...
@kwdef struct FluidRegion{N}
    element::FluidRegionElement
    lumped_T::Float64 = 50.
    e::Float64 = 1e-4
    diffusion::Bool = true
end
